...
export default function DemandDataUser() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const result = await axios('http://localhost:8090/api/v/validation/user')
      setData(result.data)
    }
    fetchData()
  }, [refresh])
  ...
  const handleSubbmitConfirmAccept = async () => {
    ...
    try {
      await axios.put('http://localhost:8090/api/v/validation/user/' + validation.id + "/VALIDEE")
  ...
  const handleSubbmitConfirmRefus = async () => {
    ...
    try {
      await axios.put('http://localhost:8090/api/v/validation/user/' + validation.id + "/REFUSEE")
  ...
  return (
    <Fragment>
      ...
      <Modal isvisible={isVisibleConfirmationAccept} onClose={() => setIsVisibleConfirmationAccept(false)}>
        <div className="p-6">
          <h3 className="text-xl font-bold text-center text-gray-900 mb-5 border-b-2 pb-7">Vous êtes sur le point d'accepter une demande de réctification Compte  </h3>
      ...
      <Modal isvisible={isVisibleConfirmationRefus} onClose={() => setIsVisibleConfirmationRefus(false)}>
        <div className="p-6">
          <h3 className="text-xl font-bold text-center text-gray-900 mb-5 border-b-2 pb-7">Vous êtes sur le point de refuser une demande de réctification Compte  </h3>
          <div className="items-center">
...
function DispalyUserData({ valid, setValidation, setIsVisibleConfirmationRefus, setIsVisibleConfirmationAccept }) {
  ...
  return (
    <tr>
      ...
      <td className='text-left px-6 py-4 whitespace-nowrap'>
        <button className=' px-4 py-1 bg-sky-500 text-white rounded-lg font-semibold text-sm '>Compte</button>
      </td>
      {(valid.etat == "REFUSEE") && (
        <>
          ...
          <td className='text-center px-6 py-4 whitespace-nowrap'>
            <div className='text-sm text-gray-500'>{valid.userPhone}</div>
          </td>
          <td className='text-center px-6 py-4 whitespace-nowrap'>
            <div className='text-sm text-gray-500'>{valid.userEmail}</div>
          </td>
        </>
      )}
