...
export default function DemandDataDiplome() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const result = await axios('http://localhost:8090/api/v/validation/diplome')
      setData(result.data)
    }
    fetchData()
  }, [refresh])
  ...
  const handleSubbmitConfirmAccept = async () => {
    ...
    try {
      await axios.put('http://localhost:8090/api/v/validation/diplome/' + validation.id + "/VALIDEE")
  ...
  const handleSubbmitConfirmRefus = async () => {
    ...
    try {
      await axios.put('http://localhost:8090/api/v/validation/diplome/' + validation.id + "/REFUSEE")
...
function DispalyUserData({ valid, setValidation, setIsVisibleConfirmationRefus, setIsVisibleConfirmationAccept }) {
  ...
  return (
    <tr>
      ...
      <td className='text-left px-6 py-4 whitespace-nowrap'>
        <button className=' px-4 py-1 bg-green-500 text-white rounded-lg font-semibold text-sm '>Diplome</button>
      </td>
      {(valid.etat == "REFUSEE") && (
        <>
          ...
          <td className='text-center px-6 py-4 whitespace-nowrap'>
            <div className='text-sm text-gray-500'>{valid.diplomeLastName}</div>
          </td>
          <td className='text-center px-6 py-4 whitespace-nowrap'>
            <div className='text-sm text-gray-500'>{valid.diplomeFirstName}</div>
          </td>
        </>
      )}
