...
export default function DemandDataPermis() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const result = await axios('http://localhost:8090/api/v/validation/permis')
      setData(result.data)
    }
    fetchData()
  }, [refresh])
  ...
  const handleSubbmitConfirmAccept = async () =>{
    ...
    try {
      await axios.put('http://localhost:8090/api/v/validation/permis/' + validation.id +"/VALIDEE")
  ...
  const handleSubbmitConfirmRefus = async () =>{
    ...
    try {
      await axios.put('http://localhost:8090/api/v/validation/permis/' + validation.id +"/REFUSEE")
  ...
  return (
    <Fragment>
      ...
      <Modal isvisible={isVisibleConfirmationAccept} onClose={() => setIsVisibleConfirmationAccept(false)}>
        <div className="p-6">
          <h3 className="text-xl font-bold text-center text-gray-900 mb-5 border-b-2 pb-7">Vous êtes sur le point d'accepter une demande de réctification Permis  </h3>
      ...
      <Modal isvisible={isVisibleConfirmationRefus} onClose={() => setIsVisibleConfirmationRefus(false)}>
        <div className="p-6">
          <h3 className="text-xl font-bold text-center text-gray-900 mb-5 border-b-2 pb-7">Vous êtes sur le point de refuser une demande de réctification Permis  </h3>
          <div className="items-center">
  ...
  const handleClickRefuser = async (valid) => {
    ...
    return (
      <tr>
        ...
        <td className='text-left px-6 py-4 whitespace-nowrap'>
        <button className=' px-4 py-1 bg-blue-500 text-white rounded-lg font-semibold text-sm '>Permis</button>
        </td>
        {(valid.etat=="REFUSEE")&&(
          <>
        ...
        <div className='text-sm text-gray-500'>{valid.numeroPermis}</div>
        </td>
        </>
        )}
